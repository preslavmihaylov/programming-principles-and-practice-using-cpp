...
double statement()
{
    ...
    */
    ...
    switch (token.kind)
    {
        case eTokenTypeInput:
            tokenString = token_str->get();
            if (tokenString.kind == eTokenTypeString)
            {
                SetInputStream(tokenString.id);
                result = 0;
            }
            else
            {
            ...
            break;
        case eTokenTypeOutput:
            tokenString = token_str->get();
            if (tokenString.kind == eTokenTypeString)
            {
                SetOutputStream(tokenString.id);
                result = 0;
            }
            else
            {
...
void calculate()
{
    ...
    while (true)
    {
        ...
        GetOutputStream() << "> ";
        token_str = new Token_stream(GetInputStream());
        ...
        try
        {
            ...
            while (currentToken.kind != eTokenTypeNull)
            {
                ...
                GetOutputStream() << "= " << result << endl;
                currentToken = token_str->get();
            }
        }
        catch (runtime_error & e)
        {
            GetOutputStream() << e.what() << endl;
            delete token_str;
            continue;
        }
